...
@app.route("/consulta7")
def consulta7():
    cur.execute(f"select pais.nombre as pais,ciudad.nombre as ciudad, (count(rentado.id_rentado)/float8((select count(rentado.id_rentado) from cliente inner join cliente_direccion on cliente_direccion.id_cliente=cliente.id_cliente inner join direccion on direccion.id_direccion=cliente_direccion.id_direccion inner join ciudad on ciudad.id_ciudad=direccion.id_ciudad inner join pais as pa on pa.id_pais=ciudad.id_pais inner join rentado on rentado.id_cliente=cliente.id_cliente where pa.id_pais=pais.id_pais group by pa.id_pais,pais.id_pais)))*100 as porcentaje_ciudad from ciudad join direccion on direccion.id_ciudad=ciudad.id_ciudad join pais on pais.id_pais=ciudad.id_pais join cliente_direccion on cliente_direccion.id_direccion=direccion.id_direccion join cliente on cliente.id_cliente=cliente_direccion.id_cliente inner join rentado on rentado.id_cliente=cliente.id_cliente group by pais.nombre,ciudad.nombre,pais.id_pais order by pais.nombre")
    datos = cur.fetchall()
    return {"datos":datos}
